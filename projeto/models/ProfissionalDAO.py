...
class ProfissionalDAO:
    ...
    def inserir(self, obj):
        if len(self.__objetos) == 0:
            ...
            maior = 0
            for profissionais in self.__objetos:
                if profissionais.get_id() > maior:
                    maior = profissionais.get_id()
            obj.set_id(maior + 1)
    ...
    def listar_nome(self, nome):
        lista = []
        for profissionais in self.__objetos:
            if profissionais.get_nome().lower().startswith(nome.lower()):
                lista.append(profissionais)
        return lista
